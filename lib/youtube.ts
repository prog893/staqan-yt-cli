...
function mapCaptionItem(item: youtube_v3.Schema$Caption): CaptionInfo {
  ...
  // `status` and `audioTrackType` are open strings in the API surface, so they
  // are narrowed against the documented enums rather than cast. An unrecognized
  // status becomes `unknown` instead of being reported as `serving`, because
  // claiming a track is live is the more damaging of the two wrong answers.
  const rawStatus = snippet.status?.toLowerCase();
  const status: CaptionTrackStatus =
    rawStatus === 'serving' || rawStatus === 'syncing' || rawStatus === 'failed'
      ? rawStatus
      : 'unknown';

  const rawAudioTrackType = snippet.audioTrackType?.toLowerCase();
  const audioTrackType: CaptionAudioTrackType =
    rawAudioTrackType === 'primary' || rawAudioTrackType === 'commentary' || rawAudioTrackType === 'descriptive'
      ? rawAudioTrackType
      : 'unknown';
  ...
  return {
    ...
    videoId: snippet.videoId!,
    language,
    // getLanguageName only maps the localization languages (issue #105), so
    // any other caption language falls back to its code rather than 'unknown'.
    languageName: getLanguageName(language) || language,
    name: snippet.name || '',
    trackKind,
    // These five were hardcoded to `false` while the API returned real values
    // in the `snippet` part already being fetched (issue #147). A draft or
    // failed track shows nothing to viewers, so fabricating `false` reported
    // captions as live when they were not.
    isClosedCaptions: snippet.isCC ?? false,
    isLarge: snippet.isLarge ?? false,
    isEasyReader: snippet.isEasyReader ?? false,
    isAutoGenerated: trackKind === 'automatic',
    isAutoSynced: snippet.isAutoSynced ?? false,
    isDraft: snippet.isDraft ?? false,
    status,
    audioTrackType,
    failureReason: snippet.failureReason || null,
