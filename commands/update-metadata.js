...
const { getVideoInfo, updateVideoMetadata } = require('../lib/youtube');
const { parseVideoId, confirm, success, error, warning, info } = require('../lib/utils');

async function updateMetadataCommand(videoId, options) {
  try {
    ...
    console.log('');
    error(`Failed to update metadata: ${err.message}`);
    process.exit(1);
  }
}

module.exports = updateMetadataCommand;
